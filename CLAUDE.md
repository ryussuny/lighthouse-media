...
## ⚡ JARVIS OS 슬래시커맨드 (v1.0 부분도입, 2026-08-06)

> 오너 제공 "LIGHTHOUSE MEDIA JARVIS OS v1.0" 문서를 부분 도입했다 — **슬래시커맨드만 신규**,
> 부서명은 새로 만들지 않고 위 COMPANY.md 기존 부서를 그대로 매핑한다(조직도 이중화 방지).
> 마스터 프롬프트·운영원칙·표준보고형식·에스컬레이션 전문은 `_JARVIS/00_회사헌장.md`.
> 회사 기억 4종(KPI·파이프라인·의사결정로그·브랜드북)은 `_JARVIS/01~04_*.md` — 매 세션 시작 시 참조.

| 커맨드 | 매핑 부서(COMPANY.md) | 용도 |
|---|---|---|
| `/전략` | 경영전략실 | 분기 OKR, 신규 수익 가설, 사업 중단 권고 |
| `/기획` | 콘텐츠제작본부(FORGE) | 소재 발굴, 제목/후킹 설계, 콘텐츠 캘린더 |
| `/대본` | 콘텐츠제작본부 — 스크립트 작가 | 영상 대본·블로그 원고·릴스 카피 |
| `/디자인` | 콘텐츠제작본부 — 비주얼 아티스트 (+ 성장마케팅본부 브랜드디자이너) | 썸네일·카드뉴스·채널아트·상품표지 |
| `/그로스` | 성장마케팅본부 | SEO·업로드 최적화·배포·채널 간 트래픽 순환 |
| `/수익` | 재무회계본부 (+ 경영전략실 CFO) | 애드센스 최적화·제휴·협찬 단가 |
| `/상품` | 출판사업부(전자책 중심) — 음원은 음악사업부, 커머스는 커머스사업부와 교차 | 신규 디지털 상품 기획 |
| `/데이터` | 경영전략실 — 데이터분석가 | 지표 수집·해석·실험 판정 |
| `/개발` | 기술운영본부 | API 연동·자동화 스크립트·대시보드 유지보수 |
| `/리스크` | 경영전략실 — 리스크관리자 (+ 기술운영본부 보안담당) | 저작권·플랫폼정책·표기의무·브랜드충돌 검토 |
| `/사역` | *(회사 범위 밖, SHEPHERD 워크스페이스 별도)* | 동산교회 사역 업무 — KPI 보고 대상 제외 |

**운영 명령어**(매일/주/월/분기): `/출근` `/퇴근` `/주간회의` `/월간결산` `/분기전략`
**품질 관문**: `/임원회의 [안건]`(부서장 반박 후 결론) · `/반박`(약점 5개) · `/검수`(출고 전 최종점검) ·
`/다시 [보수|공격]` · `/짧게` · `/완성`
**자산 명령어**: `/기록 [내용]`(→ `03_의사결정로그.md`) · `/자산` · `/재활용 [콘텐츠]`
...
```
lighthouse-media/
├── _JARVIS/          ← 회사 기억 5종(00 헌장·01 KPI·02 파이프라인·03 의사결정로그·04 브랜드북)
