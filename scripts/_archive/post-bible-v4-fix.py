...
for ds in dates:
    ...
    for p in med_paras:
        if len(p) > 130:
            med_chunks.append(p[:120])
            med_chunks.append(p[120:240])
            if len(p) > 240:
                med_chunks.append(p[240:360])
        else:
            med_chunks.append(p)
    med1 = wrap(med_chunks[0] if len(med_chunks) > 0 else '', 14)
    med2 = wrap(med_chunks[1] if len(med_chunks) > 1 else '', 14)
    med3 = wrap(med_chunks[2] if len(med_chunks) > 2 else '', 14)
    med4 = wrap(med_chunks[3] if len(med_chunks) > 3 else '', 14)
