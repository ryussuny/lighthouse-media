...
def card_meditation(lines, page, subtitle="", seed=4):
    ...
    y = 280
    for ln in lines[:12]:
        y = ctr(d, y, ln, ft_l(26), (235, 225, 205), sp=18, shadow=True, shadow_color=(15, 12, 22))
        if y > H - 120: break
...
def card_meditation2(lines, page, seed=5):
    ...
    y = 250
    for ln in lines[:12]:
        y = ctr(d, y, ln, ft_l(26), (230, 220, 200), sp=18, shadow=True, shadow_color=(15, 12, 22))
        if y > H - 120: break
...
for ds in dates:
    ...
    # ── Split meditation into paragraphs (4장에 나눠 담기) ──
    med = sec['meditation']
    med_paras = [p.strip() for p in med.split("\n\n") if p.strip()]
    if not med_paras and med:
        med_paras = [med[i:i+120] for i in range(0, len(med), 120)]
    # 묵상을 220자씩 분할 (카드 1장에 들어가는 양)
    CHARS_PER_CARD = 220
    full_med = "\n\n".join(med_paras)
    med_cards = []
    # 문단 단위로 카드에 채우기
    current = ""
    for p in med_paras:
        if len(current) + len(p) > CHARS_PER_CARD and current:
            med_cards.append(current.strip())
            current = p
        else:
            current = current + "\n\n" + p if current else p
    if current.strip():
    ...
    # 최소 2장, 최대 4장
    while len(med_cards) < 2:
        med_cards.append("")

    med1 = wrap(med_cards[0], 22)
    med2 = wrap(med_cards[1] if len(med_cards) > 1 else '', 22)
    med3 = wrap(med_cards[2] if len(med_cards) > 2 else '', 22)
    med4 = wrap(med_cards[3] if len(med_cards) > 3 else '', 22)
