...
def card_meditation(lines, page, subtitle="", seed=4):
    ...
    y = 320
    for ln in lines[:5]:
        y = ctr(d, y, ln, ft(40), (235, 225, 205), sp=30, shadow=True, shadow_color=(15, 12, 22))
...
def card_meditation2(lines, page, seed=5):
    ...
    dots(d, 130, (160, 145, 110))

    y = 300
    for ln in lines[:5]:
        y = ctr(d, y, ln, ft(40), (230, 220, 200), sp=30, shadow=True, shadow_color=(15, 12, 22))
...
for ds in dates:
    ...
    # ── Split meditation into paragraphs ──
    med = sec['meditation']
    med_paras = [p.strip() for p in med.split("\n\n") if p.strip()]
    if not med_paras and med:
        # Force split long text into chunks
        med_paras = [med[i:i+150] for i in range(0, len(med), 150)]
    med1 = wrap(med_paras[0][:140] if len(med_paras) > 0 else '', 14)
    med2 = wrap(med_paras[1][:140] if len(med_paras) > 1 else
                (med_paras[0][140:280] if len(med_paras) > 0 and len(med_paras[0]) > 140 else ''), 14)
    ...
    # Card 5: MEDITATION2 (second paragraph)
    med2_content = med2 if med2 else wrap(
        med_paras[2][:140] if len(med_paras) > 2 else med[:140], 14
    )
    if v_chunk2:
        cards.append(card_meditation2(med2_content, 5, seed=seed_base + 4))
    else:
        med3 = wrap(med_paras[2][:140] if len(med_paras) > 2 else '', 14)
        cards.append(card_meditation2(med3 if med3 else med2_content, 5, seed=seed_base + 4))

    # Card 6: QUESTION
    q1 = sec['questions'][0] if sec['questions'] else ""
    q2 = sec['questions'][1] if len(sec['questions']) > 1 else ""
    cards.append(card_question(q1, q2, 6, seed=seed_base + 5))

    # Card 7: PRACTICE
    cards.append(card_practice(sec['practice'], 7, seed=seed_base + 6))

    # Card 8: PRAYER or CLOSING
    if prayer:
        cards.append(card_prayer(pray_short, 8, seed=seed_base + 7))
    else:
        cards.append(card_closing(sec['ref'], dk, dw, seed=seed_base + 7))
